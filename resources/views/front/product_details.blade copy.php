                            ...
                            <div class="thumbs-slider">
                                <div dir="ltr" class="swiper tf-product-media-thumbs other-image-zoom"
                                    ...
                                    <div class="swiper-wrapper stagger-wrap">
                                        @foreach ($product->product_variants_imgs ?? [] as $product_variants_img)
                                            @if ($loop->last)
                                                @php $color = $product_variants_img->attribute_value->name; @endphp
                                            @endif
                                            <div class="swiper-slide stagger-item"
                                                data-color="{{ $product_variants_img->attribute_value->name ?? 'none' }}">
                                                <div class="item">
                                                    <img class="lazyload"
                                                        data-src="{{ asset($product_variants_img->img ?? '#') }}"
                                                        src="{{ asset($product_variants_img->img ?? '#') }}" alt="">
                                                </div>
                                            </div>
                                        @endforeach
                                        @foreach ($product->getMedia('main_img') ?? [] as $file)
                                            <div class="swiper-slide stagger-item" data-color="{{ $color ?? 'N/A' }}">
                                                <div class="item">
                                                    <img class="lazyload" data-src="{{ $file->getURL() ?? '#' }}"
                                                        src="{{ $file->getURL() ?? '#' }}" alt="">
                                                </div>
                                            </div>
                                        @endforeach
                                        @foreach ($product->getMedia('gallery_img') ?? [] as $file)
                                            <div class="swiper-slide stagger-item" data-color="{{ $color ?? 'N/A' }}">
                                                <div class="item">
                                                    <img class="lazyload" data-src="{{ $file->getURL() ?? '#' }}"
                                                        src="{{ $file->getURL() ?? '#' }}" alt="">
                                    ...
                                    <div class="swiper-wrapper">
                                        @foreach ($product->product_variants_imgs ?? [] as $product_variants_img)
                                            <div class="swiper-slide"
                                                data-color="{{ $product_variants_img->attribute_value->name ?? '#' }}">
                                                <a href="{{ asset($product_variants_img->img ?? '#') }}" target="_blank"
                                                    class="item" data-pswp-width="600px" data-pswp-height="800px">
                                                    <img class="tf-image-zoom lazyload"
                                                        data-zoom="{{ asset($product_variants_img->img ?? '#') }}"
                                                        data-src="{{ asset($product_variants_img->img ?? '#') }}"
                                                        src="{{ asset($product_variants_img->img ?? '#') }}"
                                                        alt="">
                                                </a>
                                            </div>
                                        @endforeach
                                        @foreach ($product->getMedia('main_img') ?? [] as $file)
                                            <div class="swiper-slide" data-color="{{ $color ?? 'N/A' }}">
                                                <a href="{{ $file->getURL() ?? '#' }}" target="_blank" class="item"
                        ...
                        <div class="tf-product-info-wrap position-relative">
                            ...
                            <div class="tf-product-info-list other-image-zoom">
                                <div class="tf-product-info-heading">
                                    ...
                                    <div class="tf-product-info-desc">
                                        <div class="tf-product-info-price">
                                            <h5 class="price-on-sale font-2"> {{ price($product->sale_price ?? 0) }} <small
                                                    style="font-size:16px;">NZD</small></h5>
                                ...
                                <div class="tf-product-info-choose-option">
                                    ...
                                    @foreach (json_decode($product->attribute_value_ids ?? '[]') as $attribute_id => $attribute_value_ids)
                                        @if ((attribute_data($attribute_id)->name ?? '') == 'Color')
                                            <div class="variant-picker-item">
                                                <div class="variant-picker-label mb_12">
                                                    {{ attribute_data($attribute_id)->name ?? 'N/A' }}:
                                                    <span
                                                        class="text-title variant-picker-label-value value-currentColor">N/A</span>
                                                </div>
                                                <div class="variant-picker-values">
                                                    @foreach ($attribute_value_ids ?? [] as $attribute_value_id)
                                                        @php
                                                            $hasStock = variant_has_stock(
                                                                $product->id,
                                                                $attribute_value_id,
                                                            );
                                                        @endphp
                                                        ...
                                                        @if ($hasStock)
                                                            <input type="radio" class="attribute_value_id"
                                                                name="attribute_{{ $attribute_key }}"
                                                                id="attribute_value_id-{{ $attribute_value_id }}"
                                                                value="{{ $attribute_value_id }}"
                                                                {{ $loop->first ? 'checked' : '' }}
                                                                onchange="get_product_variant_data({{ $product->id }})">

                                                            <label class="hover-tooltip tooltip-bot radius-60 color-btn"
                                                                for="attribute_value_id-{{ $attribute_value_id }}"
                                                                data-value="{{ attribute_value_data($attribute_value_id)->name ?? 'N/A' }}"
                                                                data-color="{{ attribute_value_data($attribute_value_id)->name ?? 'N/A' }}">
                                                                <span class="btn-checkbox"
                                                                    style="background: {{ attribute_value_data($attribute_value_id)->color_code ?? '#' }}"></span>
                                                                <span
                                                                    class="tooltip">{{ attribute_value_data($attribute_value_id)->name ?? 'N/A' }}</span>
                                                            </label>
                                                        @else
                                                            <label
                                                                class="hover-tooltip tooltip-bot radius-60 color-btn disabled">
                                                                <span class="btn-checkbox"
                                                                    style="background: {{ attribute_value_data($attribute_value_id)->color_code ?? '#' }}; opacity: 0.4;"></span>
                                                                <span
                                                                    class="tooltip">{{ attribute_value_data($attribute_value_id)->name ?? 'N/A' }}
                                                                    (Out of Stock)</span>
                                                            </label>
                                                        @endif
                                                    @endforeach
                                            ...
                                            <div class="variant-picker-item">
                                                <div class="d-flex justify-content-between mb_12">
                                                    <div class="variant-picker-label">
                                                        ...
                                                        <span class="text-title variant-picker-label-value">
                                                            {{ attribute_value_data($attribute_value_ids[0] ?? [])->name ?? 'N/A' }}
                                                        </span>
                                                    </div>
                                                </div>
                                                <div class="variant-picker-values gap12">
                                                    @foreach ($attribute_value_ids ?? [] as $attribute_value_id)
                                                        ...
                                                        @if ($hasStock)
                                                            <input type="radio" class="attribute_value_id" checked
                                                                name="attribute_{{ $attribute_key }}"
                                                                id="attribute_value_id-{{ $attribute_value_id }}"
                                                                value="{{ $attribute_value_id }}"
                                                                {{ $loop->first ? 'checked' : '' }}
                                                            ...
                                                            <label class="style-text size-btn"
                                                                ...
                                                                data-value="{{ attribute_value_data($attribute_value_id)->name ?? 'N/A' }}">
                                                                <span
                                                                    class="text-title">{{ attribute_value_data($attribute_value_id)->name ?? 'N/A' }}</span>
                                                            </label>
                                                        @else
                                                            <label class="style-text size-btn disabled">
                                                                <span
                                                                    class="text-title">{{ attribute_value_data($attribute_value_id)->name ?? 'N/A' }}</span>
                                                            </label>
                                                        @endif
                                    ...
                                    <div class="tf-product-info-quantity tf-product-info-by-btn align-items-end">
                                        <div class="title">
                                            Quantity:
                                        ...
                                        @auth
                                            <div class="w-100">
                                                ...
                                                <a href="#shoppingCart" data-bs-toggle="modal"
                                                    class="btn-style-2 flex-grow-1 text-btn-uppercase fw-6 btn-add-to-cart w-100"
                                                    onclick="add_to_cart({{ $product->id }}, 'Single', 'Add to cart')">
        ...
        function get_product_variant_data(product_id) {
            ...
            $.get('{{ route('ajax.get_product_variant_data') }}', {
                ...
                if (data.message == 'Success') {
                    let sale_price = data.product_variant.sale_price || 0;
                    let mrp_price = data.product_variant.mrp_price || 0;
                    ...
                    $('.badges-on-sale p').text(Math.round((((mrp_price - sale_price) / mrp_price) * 100) || 0) + '%');
                    $('.quantity-product').attr('max', data.current_stock || 0);
                    $('#stock_alert_msg_' + product_id + ' b').text(data.current_stock || 0);
