<form class="modal-content" action="{{ route('admin.banners.store') }}" method="post" enctype="multipart/form-data">
    ...
    <div class="modal-header">
        <h4 class="modal-title" id="mySmallModalLabel">{{ ($banner->id ?? 0) != 0 ? 'Edit':'Add' }} Banner </h4>
    ...
    <div class="modal-body dark-modal">
        <div class="row">
            <div class="col-md-12 form-group mb-3">
                ...
                <input type="text" class="form-control" name="name" id="" value="{{ $banner->name ?? '' }}" required>
            </div>
            <div class="col-md-12 form-group mb-3">
                <h6>Image</h6>
                <input type="file" class="form-control" name="main_img" id="" accept="image/*">
                <div class="img-box">
                    <a href="{{ $banner->image_url ?? asset('no_image.jpg') }}" target="_blank"><img src="{{ $banner->thumb_url ?? asset('no_image.jpg') }}" alt="" style="width: 60px; height: 60px; object-fit: contain"></a>
                </div>
            </div>
            ...
            <div class="col-md-6 form-group">
                <h6>Status</h6>
                <label class="switch">
                    <input type="checkbox"  name="status" value="1" {{ ($banner->status ?? 1) == 1 ? 'checked':'' }}><span class="switch-state"></span>
                </label>
            </div>
        </div>
    </div>
